...
if(count($query->fetch_array(MYSQLI_NUM))) {
    echo '<html><head><title>TOP '.$max.' DE COMANDANTES</title></head><body bgcolor="#212121"><table border="0" cellpadding="5" cellspacing="0" style="box-shadow: 0px 0px 8px white; border-radius: 5px; width: 100%;min-width: 50%;">
            <tbody style="text-align:center;color:white;">
                <tr style="font-weight: bold;background-color: black;">
                    ...
                    <td>
                        MATADOS DE COMANDANTE
                    </td>
                </tr>';
                $n = 1;
                $query = $db->query('SELECT name, comandante_kills FROM `'.$database['tabla'].'` ORDER BY comandante_kills DESC, name ASC LIMIT 0,'.$max) or die ("Error en la consulta: ".mysqli_error($db));
